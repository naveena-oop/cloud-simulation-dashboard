...
import { 
  ...
  Settings2, Tag, Trash2, FolderPlus, List, Star, MessageSquare 
} from 'lucide-react'
...
export const AzureAppServices: FC<AzureViewProps> = ({ navigateTo }) => {
  const { setTerminalOpen, executeCommand } = useCloudStore()

  const handleCreate = () => {
    setTerminalOpen(true)
    setTimeout(() => executeCommand('Azure', 'az webapp create --name newwebapp --resource-group default --plan myAppServicePlan'), 500)
  }
    ...
    <div className="flex flex-col bg-white text-[#323130] min-h-[calc(100vh-48px)] font-sans">
      <div className="px-6 pt-4 pb-2 flex-1 flex flex-col">
        ...
        <div className="flex items-center gap-1 mb-3 text-[13px] text-[#605E5C]">
          ...
          <span className="hover:underline cursor-pointer">App Services</span>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between mb-2 gap-4">
          <div className="flex items-start gap-4">
            <h1 className="text-2xl font-semibold opacity-90 tracking-tight flex items-center gap-3">
              App Services <Star size={16} className="text-gray-400 cursor-pointer hover:text-gray-600 outline-none" fill="transparent" />
            </h1>
          </div>
          {/* AI Chips */}
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-white border border-[#EDEBE9] rounded-full px-3 py-1 text-[12px] hover:bg-[#F3F2F1] cursor-pointer shadow-sm">
              Identify availability risks for apps
            </div>
            <div className="flex bg-white border border-[#EDEBE9] rounded-full px-3 py-1 text-[12px] hover:bg-[#F3F2F1] cursor-pointer shadow-sm">
              Mitigate potential CPU problems for apps
            </div>
            <div className="flex bg-white border border-[#EDEBE9] rounded-full px-3 py-1 text-[12px] hover:bg-[#F3F2F1] cursor-pointer shadow-sm">
              Best practices for managing web apps
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between border-b border-[#EDEBE9] py-2 outline-none w-full relative">
          <div className="flex items-center gap-4 text-[13px]">
            ...
            <button className="flex items-center gap-1.5 text-[#0078D4] hover:underline whitespace-nowrap"><Trash2 size={14}/> Manage Deleted Apps</button>
        ...
        <div className="flex items-center gap-2 py-3 overflow-x-auto border-b border-[#EDEBE9]">
           ...
           <div className="flex items-center gap-1.5 bg-[#F3F2F1] text-[13px] rounded px-3 py-1 cursor-pointer hover:bg-[#EDEBE9] shrink-0 border border-transparent hover:border-gray-300">
             Type <span className="font-semibold">equals all</span> <span className="ml-1 text-gray-500 hover:text-black">X</span>
           </div>
           <div className="flex items-center gap-1.5 bg-[#F3F2F1] text-[13px] rounded px-3 py-1 cursor-pointer hover:bg-[#EDEBE9] shrink-0 border border-transparent hover:border-gray-300">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center min-h-[400px]">
          <div className="mb-6 opacity-40 mt-[-60px]">
             {/* App Services Icon Custom SVG (Globe) */}
             <svg width="84" height="84" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="32" cy="32" r="24" fill="#E1DFDD" />
                <path d="M12 32A20 20 0 0 0 52 32A20 20 0 0 0 12 32Z" fill="#C8C6C4" />
                <circle cx="22" cy="38" r="6" fill="white" />
                <circle cx="42" cy="38" r="4" fill="white" />
                <circle cx="36" cy="22" r="8" fill="white" />
                <path d="M22 38L36 22L42 38Z" stroke="white" strokeWidth="2" fill="transparent" />
             </svg>
          </div>
          <h2 className="text-[20px] font-semibold text-[#323130] mb-3">No app services to display</h2>
